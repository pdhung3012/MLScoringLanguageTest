...
os.chdir(cwd)
# base_directory = cwd
base_directory = '/home/hung/git/resultETI/d2vAndDeep_tt/'
print(base_directory)

# directory = cwd + '/Data'
# clean_directory = cwd + '/Clean_Data'
# directory = base_directory + '/Data'
train_directory = base_directory + '/vectorTrain/'
test_directory = base_directory + '/vectorTest/'
...
for filename in os.listdir(train_directory):
    print(filename)
    typeName = filename.replace('_10cv.csv', '')
    data = pd.read_csv(train_directory + "/" + filename)
    createDir(log_directory)
    # typeName = 'BI'
    ...
    dataTest = pd.read_csv(test_directory + "/" + typeName+'_test.csv')
    lb = LabelBinarizer()  # for one-hot encoding of response
    dataTest['score'] = lb.fit_transform(dataTest['score']).tolist()
    labelTest = [dataTest['score'][i].index(1) for i in range(dataTest.shape[0])]
    labelTest = np.asarray(labelTest)
    featuresTest = dataTest.drop(['no', 'score'], axis=1)
    ...
    train_X, test_X = features, featuresTest
    train_y, test_y = label, labelTest
