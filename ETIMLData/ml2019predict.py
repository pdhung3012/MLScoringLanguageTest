...
# set file directory
fpInput='/home/hung/git/confres/eti/code/step3/4GramYearVector/'

# load data for 10-fold cv
df_all = pd.read_csv(fpInput+'output_Response4Gram_2018.csv')
...
all_data = df_all.drop([' expected',' predicted',' maxSim'],axis=1)

df_test = pd.read_csv(fpInput+'output_Response4Gram_2019.csv')
test_label = df_test[' expected']
# all_data = df_all.drop(['label','maxSim','maxSim-r2','maxSim-r3','maxSim-r4','maxSim-p1','maxSim-p2','maxSim-p3','maxSim-p4'],axis=1)
test_data = df_test.drop([' expected',' predicted',' maxSim'],axis=1)
