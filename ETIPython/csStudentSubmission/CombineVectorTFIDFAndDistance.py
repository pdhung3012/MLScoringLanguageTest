...
def combineData(fpInputSubmission, fopTextFolder,fpInputDistanceFromPrevious, fpOutputVectorTFIDF):
    ...
    distance_csv=pd.read_csv(fpInputDistanceFromPrevious)
    columnA=distance_csv.distA
    columnAMinus = distance_csv.distAMinus
    columnBPlus = distance_csv.distBPlus
    columnB = distance_csv.distB
    # columnBMinus = distance_csv.distBMinus
    ...
    columnTDFTitle ="no,reviseNetID,expected,distA,distAMinus,distBPlus,distB,distBMinus,"
    ...
    for i in range(len(corpus)):
        ...
        # csv.write(row)
        rowTFIDF=str(indexCorpus+1)+ ',' + str(strTestId)+',' + str(expectedResult) + ',' + str(columnA[indexCorpus]) + ',' + str(columnAMinus[indexCorpus]) + ',' + str(columnBPlus[indexCorpus]) \
                 + ',' + str(columnB[indexCorpus])  + ',' +strVectorContent+ '\n'
...
def main():
    ...
    combineData(fpStudentSubmission, fopText,fpInputDistanceFromPrevious, fpStudentTFIDF)
